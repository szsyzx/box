...
download_xui(){
    ...
    if [ $# == 0 ]; then
        ...
        if [[ -z "$last_version" ]]; then
            ...
            last_version=$(curl -sm8 https://raw.githubusercontents.com/Altria002/Misaka-blog/x-ui/main/config/version)
            if [[ -z "$last_version" ]]; then
        ...
        wget -N --no-check-certificate -O /usr/local/x-ui-linux-$(archAffix).tar.gz https://github.com/FranzKafkaYu/x-ui/releases/download/${last_version}/x-ui-linux-$(archAffix).tar.gz
        if [[ $? -ne 0 ]]; then
    ...
    wget -N --no-check-certificate https://raw.githubusercontents.com/Altria002/Misaka-blog/x-ui/main/x-ui.sh -O /usr/bin/x-ui
